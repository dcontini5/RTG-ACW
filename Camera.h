...
public:
	...
	explicit Camera(const std::vector<CameraCoordinates>& cameras): _camera1(cameras[0]), _camera2(cameras[1]), _activeCamera(true) {};
	~Camera();

	
	
	void SwapCamera(const int& i) { _activeCamera = i; }
	XMVECTOR GetEye() const { return _activeCamera ? _camera1.Eye : _camera2.Eye; }
	XMVECTOR GetFront() const { return _activeCamera ? _camera1.Front : _camera2.Front; }
	XMVECTOR GetUp() const { return _activeCamera ? _camera1.Up : _camera2.Up; }
	XMVECTOR GetRot() const { return _activeCamera ? _camera1.Rot : _camera2.Rot; }
